...
def video_feed():
    ...
    if windowMode:
        ...
        return Response(generate(),
                        mimetype="multipart/x-mixed-replace; boundary=frame")  # return the output from the generate() function
    else:
        return Response(generate(),
                        mimetype="multipart/x-mixed-replace; boundary=frame")  # return the output from the generate() function
            ...
            with open('session.json') as json_file:  # open the saved file
                ...
                for op in temp_operations:  # for operation in loaded operations
                    ...
                    for temp_input in op["check_in"]:
                        checkinputs.append(
                            operation_CheckboxInput(temp_input["name"], temp_input["text"], temp_input["options"],
                                                    temp_input["style"], temp_input["textStyle"],
                                                    temp_input["optionTextStyle"], temp_input["value"],
                                                    temp_input["direction"], temp_input[
                    ...
                    for temp_output in op["var_out"]:
                        varOutputs.append(
                            operation_TextInput(temp_output["name"], temp_output["text"], temp_output["style"],
                                                temp_output["textStyle"], temp_output["value"],
...
def add_operation(operation_name):  # add a new operation
    ...
    new_op = copy.deepcopy(
        sky_operations[request.form["action"]])  # copy an operation from the dictionary of operations
